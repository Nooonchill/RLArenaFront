...
const UserStat = ({ user, isLogged }) => {
  ...
  return (
    <div className="bg-lightwhiteturquoise dark:bg-dark pt-2 rounded-3xl mb-6 text-center text-dark dark:text-lightwhiteturquoise">
      {!isLogged ? (
        <h1 className="text-dark dark:text-lightwhiteturquoise text-4xl">Добро пожаловать!</h1>
      ) : (
        <div>
          <h1 className="text-4xl mb-1">Добро пожаловать, {user.username}!</h1>
          <div className="flex justify-around items-stretch space-x-4 f flex-wrap">
            <div className="bg-none p-2 m-1 min-w-[190px] w-1/4 items-center justify-center rounded-full flex flex-col">
              <span className="text-3xl">#{user.stats.position}</span>
            ...
            <div className="bg-none p-2 rounded-full m-1 min-w-[190px] w-1/4 items-center justify-center flex flex-col ">
              <span className="text-3xl ">{changeTimeView(user.stats.timedelta)}</span>
